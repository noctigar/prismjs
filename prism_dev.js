/* PrismJS 1.9.0
http://prismjs.com/download.html?themes=prism-okaidia&languages=markup+css+clike+javascript+actionscript+c+aspnet+bash+basic+cpp+csharp+coffeescript+ruby+diff+git+go+haml+http+java+json+kotlin+latex+markdown+objectivec+perl+php+php-extras+python+sass+scss+scheme+smarty+sql+swift+vbnet+vim+yaml&plugins=line-highlight+line-numbers+toolbar+keep-markup+show-language+copy-to-clipboard */
...
Prism.plugins.toolbar.registerButton('show-language', function(env) {
	var pre = env.element.parentNode;
	if (!pre || !/pre/i.test(pre.nodeName) || pre.getAttribute('data-language') === "none") {
		return;
	}
